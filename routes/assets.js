...
router.get('/', setAuth, async (req, res) => {
    ...
    const _stocks = assets.filter(
        (e) => e.balance !== 0 && e.assetType === 'stock'
    );
    const _coins = assets.filter(
        (e) => e.balance !== 0 && e.assetType === 'cryptocoin'
    );
    const assetsObj = {};
...
router.put('/coin/:coinName/add', setAuth, async (req, res) => {
    ...
    try {
        ...
        return res.status(400).send({
            error: `The coin with the name '${coinName}' already exists.`,
        });
    }
});
...
router.post('/stock/:ticker/buy', setAuth, async (req, res) => {
    ...
    if (!asset)
        return res.status(404).send({
            error: `${ticker} not registered by now. Please press add button to create a stock`,
        });
    const { averagePrice, balance } = asset;
    asset.averagePrice =
        (balance * averagePrice + quantity * purchasePrice) /
        (balance + quantity);
    asset.balance += quantity;
    await asset.save();

    let stockHistoryData = fs
        .readFileSync('./datas/stocksHistory.json')
        .toString();
    ...
    fs.writeFileSync(
        './datas/stocksHistory.json',
        JSON.stringify(stockHistoryData)
    );
...
router.post('/coin/:coinName/buy', setAuth, async (req, res) => {
    ...
    if (!asset)
        return res.status(404).send({
            error: `${coinName} not registered by now. Please press add button to create a coin`,
        });
    console.log(asset);
    const { averagePrice, balance } = asset;
    asset.averagePrice =
    ...
    let coinHistoryData = fs
        .readFileSync('./datas/coinsHistory.json')
        .toString();
    ...
    coinHistoryData.push({
        ...
        purchasePrice,
        quantity,
        dataOfPurchase: new Date(),
        user,
    });
    fs.writeFileSync(
        './datas/coinsHistory.json',
        JSON.stringify(coinHistoryData)
    );
    return res.send({ asset, purchasePrice, quantity });
    s;
});
...
router.post('/stock/:ticker/sell', setAuth, async (req, res) => {
    ...
    if (balance - quantity < 0)
        return res.status(400).send({ error: "You don't have enough stocks." });
...
router.post('/coin/:coinName/sell', setAuth, async (req, res) => {
    ...
    if (!asset)
        return res
            .status(404)
            .send({ error: `There is no such coin '${coinName}'` });
    const { balance, averagePrice } = asset;
    if (balance - quantity < 0)
        return res.status(400).send({ error: "You dont't have enough coins." });
    else asset.balance -= quantity;
    await asset.save();
