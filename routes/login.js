...
router.post('/', async(req, res) => {
    ...
    if(user===null) return res.status(404).send({error : 'User is not found'});
    else if(user.password!==encryptedPassword)  return res.status(403).send({error : 'Wrong Password'});
